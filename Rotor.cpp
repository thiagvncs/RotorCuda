...
void Rotor::getGPUStartingKeys(Int & tRangeStart, Int & tRangeEnd, int groupSize, int nbThread, Int * keys, Point * p)
{
	if (rKey > 0) {
		...
		for (int i = 0; i < nbThread; i++) {
			...
			// For the last GPU thread, ensure the first kernel batch covers the END boundary
			if (i == nbThread - 1) {
				keys[i].Set(&tRangeEnd);
				keys[i].Sub((uint64_t)(STEP_SIZE - 1));
			} else {
            ...
            Int dobb;
            // For the last thread, ensure we cover up to the exact provided end of range
            if (i == nbThread - 1) {
                dobb.Set(&tRangeEnd);
            } else {
					...
					if (nbit2 < 1) {
						...
						if (rKey > 0) {
							...
							if (isAlive(params)) {
								...
								if (avgGpuKeyRate > 1000000000) {
									...
									printf("\r  [%s] [R: %" PRIu64 "] %s [F: %d] [GPU: %.2f Gk/s] [T: %s]  ",
									...
									memset(timeStr, '\0', 256);
									printf("\r  [%s] [R: %" PRIu64 "] %s [F: %d] [GPU: %.2f Mk/s] [T: %s]  ",
										toTimeStr(t1, timeStr),
										rKeyCount,
										rhex.GetBase16().c_str(),
										nbFoundKey,
						...
						if (rKey > 0) {
							if (isAlive(params)) {
								...
								if (avgGpuKeyRate > 1000000000) {
									memset(timeStr, '\0', 256);
									printf("\r  [%s] [R: %" PRIu64 "] %s [F: %d] [CPU %d: %.2f Gk/s] [T: %s]  ",
										toTimeStr(t1, timeStr),
										rKeyCount,
										rhex.GetBase16().c_str(),
										nbFoundKey,
									...
									memset(timeStr, '\0', 256);
									printf("\r  [%s] [R: %" PRIu64 "] %s [F: %d] [CPU %d: %.2f Mk/s] [T: %s]  ",
										toTimeStr(t1, timeStr),
										rKeyCount,
										rhex.GetBase16().c_str(),
										nbFoundKey,
